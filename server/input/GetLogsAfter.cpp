...
bool GetLogsAfter::parse()
{
    ...
    if (input.length() - static_cast<cm::Index>(cm::DATE_TIME_TEMPLATE.length()) != 13) {
        return false;
    }

    borderMoment = QDateTime::fromString(input.mid(static_cast<int>(lim + 1)) , cm::DATE_TIME_TEMPLATE.c_str());
